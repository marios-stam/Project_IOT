...
def load_key():  # load key from .txt file
    with open(os.path.join(os.getcwd(), "server/trucks/truck_routing/mapbox/api_key.txt"), "r") as key_file:
        key = key_file.read()
    return key
...
def get_solution(url):
    ...
    # save to json file
    result_file_name = os.path.join(os.getcwd(), "server/trucks/truck_routing/mapbox/result.json")
